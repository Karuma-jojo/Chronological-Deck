import {STORAGE_KEY,emptyEvidence,validateEvidence,mergeEvidence,expose,taskText,reviewQueue,moduleEvidenceSummary,compilerPacket,evidenceIsCurrent} from './core.js';

const $=id=>document.getElementById(id);
const tell=x=>$('status').textContent=x;
const uuid=()=>crypto.randomUUID();
let course,roadmap,state,session,problemId,lastSaved=null,keys=null,visit=0,noteSeen=false,referenceBefore=false,storageOK=true;
...
function renderQueue(){
 const rows=reviewQueue(course,state).slice(0,12);$('queue').replaceChildren(...rows.map(r=>button(`${String(r.order).padStart(2,'0')} · ${r.reason}${r.due?' · '+new Date(r.due).toISOString().slice(0,10):''}`,()=>selectSession(r.order))));
}
function renderModuleEvidence(){
 const s=moduleEvidenceSummary(course,state);put('evidenceSummary',`${s.both}/${s.total} sessions currently have fresh independent secure evidence on both main and transfer tasks. This is evidence coverage, not automatic M01 clearance.`);
}
function renderHistory(){
 const items=state.attempts.filter(a=>a.problemId===problemId).slice(-12).reverse();
 $('history').replaceChildren(...items.map(a=>{const el=document.createElement('article');const current=evidenceIsCurrent(a,course);el.textContent=`${a.at} · ${a.assistance} · ${a.result}${a.reviewOf?' · review':''}${current?'':' · STALE CONTRACT'}\n${a.referenceSeenBefore?'Reference was already exposed. ':''}${a.noteSeenDuringAttempt?'Learning note used. ':''}\n${a.answer}`;return el;}));
}
...
function showProblem(id){
 ...
 const p=course.problems[id],prior=state.exposures[id];referenceBefore=!!prior?.referenceSeenAt;
 put('taskMeta',`${p.kind.toUpperCase()} · ${id}`);put('problem',taskText(p));
 put('exposure',prior?`Previously displayed in this browser log (${prior.views} view${prior.views===1?'':'s'}).${prior.referenceSeenAt?' Reference has been exposed before.':''}`:'First display in this browser log. Outside exposure is unknown.');
...
async function reveal(){
 ...
 try{const refs=await evaluator();if(problemId!==id||visit!==token||lastSaved!==saved)return;const r=refs[id];
  put('reference',r.reference+'\n\n'+r.rubric.map(x=>`${x.points} points: ${x.criterion}`).join('\n')+'\n\n'+r.marking);$('reference').hidden=false;expose(state,id,undefined,'referenceSeenAt');persist('Evaluator reference opened; exposure recorded.');
 }catch(e){tell(e.message);}
}
async function copy(text){try{await navigator.clipboard.writeText(text);tell('Copied.');}catch{const area=document.createElement('textarea');area.value=text;area.readOnly=true;area.setAttribute('aria-label','Copy text manually');$('status').replaceChildren('Clipboard unavailable. Copy below:',area);area.select();}}
...
async function init(){
 ...
 $('save').onclick=()=>{
  ...
  const revealedSince=!!state.exposures[problemId]?.referenceSeenAt&&!referenceBefore;const assistance=revealedSince?'revealed':noteSeen&&$('assistance').value==='independent'?'guided':$('assistance').value;
  const a={id:uuid(),problemId,at:new Date().toISOString(),answer,assistance,minutes,result:'unreviewed',error:'',referenceSeenBefore:referenceBefore,noteSeenDuringAttempt:noteSeen,contractHash:session.contractHash};state.attempts.push(a);lastSaved=a.id;$('reveal').disabled=false;$('review').disabled=false;persist('Attempt saved. No mastery clearance was granted.');renderHistory();renderQueue();renderModuleEvidence();
 };
 $('reveal').onclick=reveal;
 $('saveReview').onclick=()=>{const a=state.attempts.find(x=>x.id===lastSaved);if(!a||a.problemId!==problemId)return;state.attempts.push({...a,id:uuid(),at:new Date().toISOString(),reviewOf:a.id,result:$('result').value,error:$('error').value});persist('Review saved as a separate evidence record.');renderHistory();renderQueue();renderModuleEvidence();};
 $('copyTask').onclick=()=>copy('[T22 ELITE — LEARNER TASK]\n\n'+taskText(course.problems[problemId]));
 $('copyPacket').onclick=async()=>{try{const refs=await evaluator();await copy(compilerPacket(course,session,refs,problemId));expose(state,problemId,undefined,'packetExportedAt');persist();}catch(e){tell(e.message);}};
 ...
 if(storageOK)tell(`Ready: ${course.sessions.length} authored M01 sessions, ${Object.keys(course.problems).length} original tasks, 65-module roadmap.`);
}
