...
async function init(){
 ...
 $('note').onclick=()=>{put('learning','LEARNING NOTE — ASSISTANCE, NOT INDEPENDENT EVIDENCE\n\n'+session.lesson);$('learning').hidden=false;const hasGuide=typeof session.guidedFeedback==='string'&&session.guidedFeedback.length>0;$('guidedPanel').hidden=!hasGuide;$('guidedFeedback').hidden=true;put('guidedFeedback','');$('guidedAnswer').value=hasGuide?(guidedDrafts.get(session.id)||''):'';$('guidedCheck').disabled=!$('guidedAnswer').value.trim();noteSeen=true;if($('assistance').value==='independent')$('assistance').value='guided';const e=expose(state,problemId,undefined,'lessonSeenAt');e.lessonContentVersion=session.instructionVersion||course.instructionVersion||course.version;persist('Learning note opened; assistance exposure recorded.');};
