...
function render(){
  ...
  layer.innerHTML=`<select id="t25AtomicSelect" hidden><option value=""></option>${T25_ATOMIC_CARDS.map(c=>`<option value="${esc(c.id)}">${esc(c.id)}</option>`).join("")}</select>
    ...
    <div class="t25-actions"><button type="button" id="t25AtomicCopy">Copy authored session card</button><a href="${LAMBDA_COMPILER_URL}" target="_blank" rel="noopener noreferrer">Open λ Compiler</a><a href="${LAMBDA_EXTRACTOR_URL}" target="_blank" rel="noopener noreferrer">Open λ ARC Extractor</a></div>
