...
| Kazunga/Bansilal and related determinant/matrix misconception studies | Determinant/matrix misconception comparator | Avoid “determinant = formula” and “square = invertible”; require structural claims to name their hypotheses and what determinant does **not** establish (e.g. numerical stability). | Samples are context-specific; exact misconception frequencies are not imported. |

### Pedagogy-Evidence Ledger

| Concept | Documented learner difficulty / context | False model to attack | Design consequence |
| --- | --- | --- | --- |
| Linear transformations | Research syntheses and Turgut et al.: students can struggle moving between function, graphical and matrix registers and may demand an explicit formula even when basis action suffices. | “A linear map is just a matrix/formula” or “straight lines stay straight, so it is linear.” | S01 begins with map laws before matrices; assessments include basis-action and affine/nonlinear counterexamples. |
