...
{
 const id='T22V3::SIDE276::S09-T@2',C=[[2,-1,1],[1,1,0]],p=[0,2,1],q=[1,1,-2],n=[1,-1,-3];
 assert.deepEqual(mv(C,p),[-1,2]);
 assert.deepEqual(mv(C,q),[-1,2]);
 assert.deepEqual(q.map((v,i)=>v-p[i]),n);
 assert.deepEqual(mv(C,n),[0,0]);
 for(const t of [-3,0,1,4])assert.deepEqual(mv(C,add(p,sm(t,n))),[-1,2]);
 contains(id,'q-p=(1,-1,-3)^T','every solution is (t,2-t,1-3t)=p+t(q-p)','C(x-p)=0');
}
