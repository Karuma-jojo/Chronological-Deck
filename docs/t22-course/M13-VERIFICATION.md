...
The prior `c40a175` implementation had a successful [Actions run 36099514227](https://github.com/Karuma-jojo/Chronological-Deck/actions/runs/36099514227), including Chromium. For this repaired candidate, 49 syntax checks and 39 structural/semantic commands pass locally, including new guards for the adversarial findings. The validator checks structure, exact claim-to-prompt/rubric links, selected numeric reconstruction and bounded known-bad controls; it cannot certify all prose teaching or freshness. Local Chromium is unavailable, so the repaired-head Actions run remains the required browser result. Record its actual URL after push.
