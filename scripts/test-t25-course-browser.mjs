...
try{
 ...
 const leanAvailable=existsSync('course/generated/runtime.json');
 ...
 if(leanAvailable){
  assert(requests.some(x=>x.endsWith('/course/generated/runtime.json')),'Lean runtime was not used');
  assert(!requests.some(x=>x.endsWith('/course/generated/course.json')),'Full audit course fetched during first paint');
  assert(!requests.some(x=>x.endsWith('/course/generated/notes.json')),'Learning-note bank fetched during first paint');
 }
 ...
 await page.click('#note');if(leanAvailable)assert(requests.some(x=>x.endsWith('/course/generated/notes.json')),'Learning-note bank was not lazy-loaded on request');assert.equal(await page.locator('#assistance').inputValue(),'guided');await page.fill('#answer','Working after learning note.');await page.selectOption('#assistance','independent');await page.click('#save');assert.equal((await page.evaluate(()=>JSON.parse(localStorage.getItem('chrono_t25_course_evidence_v1')))).attempts.at(-1).assistance,'guided');
 ...
 const corrupt=await browser.newContext();await corrupt.addInitScript(()=>localStorage.setItem('chrono_t25_course_evidence_v1','{broken'));const cp=await corrupt.newPage();await cp.goto(base+'/t25-course.html');await cp.waitForSelector('#session option',{state:'attached'});assert((await cp.locator('#status').textContent()).includes('not been overwritten'));assert.equal(await cp.evaluate(()=>localStorage.getItem('chrono_t25_course_evidence_v1')),'{broken');await corrupt.close();results.push('Corrupt evidence preserved');
