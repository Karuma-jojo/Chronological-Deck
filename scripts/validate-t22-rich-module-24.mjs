...
expect(Boolean(WORLD.terminals.find((terminal) => terminal.id === "T22")), "T22 terminal must be loaded before M24 validation.");
...
if (errors.length) {
  console.error(`T22 M24 rich validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log(`T22 M24 rich syllabus OK: ${expectedIds.length} stable mission cards validated.`);
