...
const moduleId = "ARC513";
const expectedIds = [
  "T22-M34-A01", "T22-M34-A02", "T22-M34-A03", "T22-M34-A04", "T22-M34-A05",
  "T22-M34-A06", "T22-M34-A07", "T22-M34-A08", "T22-M34-A09",
];
...
expect(Boolean(WORLD.terminals.find((terminal) => terminal.id === "T22")), "T22 terminal must be loaded before M34 validation.");
...
expect(richModule?.arcs?.["T22-M34-A01"]?.requiredMastery?.some((item) => item.includes("wrong mathematical object")), "M34 A01 must diagnose target/simulation mismatch.");
expect(richModule?.arcs?.["T22-M34-A03"]?.requiredMastery?.some((item) => item.includes("n^{-1/2}")), "M34 A03 must operationalize square-root Monte Carlo error.");
expect(richModule?.arcs?.["T22-M34-A06"]?.requiredMastery?.some((item) => item.includes("negative covariance")), "M34 A06 must justify antithetic variance reduction through covariance.");
expect(richModule?.arcs?.["T22-M34-A07"]?.requiredMastery?.some((item) => item.includes("Cov(Y,C)/Var(C)")), "M34 A07 must derive the optimal control-variate coefficient.");
expect(richModule?.arcs?.["T22-M34-A08"]?.requiredMastery?.some((item) => item.includes("support")), "M34 A08 must defend importance-sampling support conditions.");
expect(richModule?.arcs?.["T22-M34-A09"]?.nextArcBoundary?.includes("ARC541"), "M34 A09 must hand multivariate random-vector geometry to ARC541.");
expect(richModule?.explicitlyOutOfScope?.some((item) => item.includes("ARC537")), "M34 must defer bootstrap/permutation inference to ARC537.");
expect(richModule?.explicitlyOutOfScope?.some((item) => item.includes("ARC524")), "M34 must defer Markov-chain/MCMC foundations to ARC524.");
expect(richModule?.explicitlyOutOfScope?.some((item) => item.includes("ARC560")), "M34 must defer terminal backtest integration to ARC560.");

if (errors.length) {
  console.error(`T22 M34 rich validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log(`T22 M34 rich syllabus OK: ${expectedIds.length} stable Monte Carlo mission cards validated.`);
