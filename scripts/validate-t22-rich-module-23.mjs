...
if (errors.length) {
  console.error(`T22 M23 rich validation failed with ${errors.length} issue(s):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}
console.log(`T22 M23 rich syllabus OK: ${expectedIds.length} stable mission cards validated.`);
