-- Chrono-Deck private Archive admin RPC surface
-- Reconciled from live Supabase migration:
--   20260821105112 arc_archive_private_api_v1
--
-- IMPORTANT:
-- These are the CURRENT live definitions, including the later academic
-- clearance completion semantics. They are intentionally source-controlled as
-- a reproducibility snapshot; later migrations may still replace them.
--
-- The functions are callable only by service_role. The public Archive MCP
-- reaches them through the authenticated arc-archive-access Edge Function.

begin;

create or replace function public.chrono_load_arc_bundle_admin(
  p_user_id uuid,
...
as $$
  ...
  scored as (
    select
      ...
      greatest(
        -1.0,
        least(1.0, 1.0 - (e.embedding <=> p_query_embedding))
  ...
  select
    ...
    s.semantic_score,
    (
      s.raw_lexical_score / (1.0 + s.raw_lexical_score)
...
create or replace function public.chrono_semantic_embedding_status_admin(
  p_user_id uuid
)
returns jsonb
language sql
...
revoke all on function public.chrono_load_arc_bundle_admin(uuid, text)
  from public, anon, authenticated;
revoke all on function public.chrono_hybrid_search_arc_chunks_admin(
  uuid, text, extensions.vector, text, text, boolean, integer
) from public, anon, authenticated;
revoke all on function public.chrono_semantic_embedding_status_admin(uuid)
  from public, anon, authenticated;
revoke all on function public.chrono_semantic_related_arcs_admin(uuid, text, integer)
  from public, anon, authenticated;

grant execute on function public.chrono_load_arc_bundle_admin(uuid, text)
  to service_role;
grant execute on function public.chrono_hybrid_search_arc_chunks_admin(
  uuid, text, extensions.vector, text, text, boolean, integer
) to service_role;
grant execute on function public.chrono_semantic_embedding_status_admin(uuid)
  to service_role;
grant execute on function public.chrono_semantic_related_arcs_admin(uuid, text, integer)
  to service_role;
