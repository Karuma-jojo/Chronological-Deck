...
--
-- Goals:
--   1. avoid creating a new ARC revision when only chrono_* bookkeeping changed;
--   2. update ARC sections incrementally so unchanged sections keep their embeddings;
--   3. keep a lightweight storage-health report available to the Obsidian bridge;
--   4. provide an explicit all-ARC revision-pruning helper;
--   5. remove one redundant revision index.
--
-- Run after arc-clearance-semantic-completion-v1.sql and obsidian-reader-v3.sql.

begin;

-- The unique (user_id, arc_id, revision) index already supports forward/backward
-- scans, so the extra DESC index duplicates the same B-tree ordering information.
drop index if exists public.arc_revisions_user_arc_revision_idx;

-- Match the Obsidian-side fingerprint rule closely enough to identify a true
-- content no-op. Machine-managed chrono_* frontmatter values are intentionally
-- ignored; line endings and trailing whitespace are normalized.
...
-- Compact user-facing report. Relation byte counts are project-wide relation
-- sizes; row/content counts are scoped to the signed-in user.
...
as $$
  select jsonb_build_object(
    'logicalArcs', (
      select count(distinct d.logical_arc_id)
    ...
    'embeddingReady', (
      select count(*)
      from public.arc_section_embeddings e
      where e.user_id = (select auth.uid()) and e.embedding is not null
    ),
    'embeddingPending', (
      select count(*)
      from public.arc_section_embeddings e
      where e.user_id = (select auth.uid()) and e.embedding is null
    ),
    'revisions', (
      select count(*)
    ...
    'markdownBytes', (
      select coalesce(sum(octet_length(d.source_markdown)), 0)
      from public.arc_documents d
      where d.user_id = (select auth.uid())
    ),
    'revisionSnapshotBytes', (
      select coalesce(sum(pg_column_size(r.snapshot)), 0)
      from public.arc_revisions r
      where r.user_id = (select auth.uid())
    ),
    'mediaItems', (
...
-- Explicit maintenance helper. This is NOT run automatically by the migration.
-- It keeps revision 1, the newest p_keep_recent snapshots, and every
-- p_keep_every-th older milestone through the existing per-ARC pruning rule.
...
-- Incremental Obsidian sync.
-- Unchanged section rows are left in place, which means their semantic chunks
-- and embeddings are also left in place. Only new/changed/deleted H2 sections
-- cause embedding work. A content no-op does not create a new revision.
...
begin
  ...
  -- Do not mint a revision or rebuild sections/embeddings if the only changes
  -- are chrono_* bookkeeping fields. Keep source path/source Markdown fresh.
  if v_previous_revision is not null
     and public.chrono_markdown_content_hash(v_existing_markdown)
         = public.chrono_markdown_content_hash(v_incoming_markdown)
  then
  ...
  -- 1) Insert new sections only. INSERT triggers semantic chunk creation.
  with incoming as (
    select
  ...
  -- 2) Update rows whose searchable content actually changed. The existing
  -- UPDATE OF heading/content trigger re-seeds embeddings only for these rows.
  with incoming as (
    select
      coalesce(nullif(section.value->>'id', ''), 'section-' || section.ordinality::text) as section_id,
      coalesce(nullif(section.value->>'type', ''), 'notes') as type,
  ...
  where s.user_id = v_user_id
    ...
    and (
      s.heading is distinct from i.heading
      or s.content_markdown is distinct from i.content_markdown
    );

  -- 3) Metadata-only changes do not touch heading/content, so they do not
  -- cause semantic re-embedding.
  with incoming as (
    select
      coalesce(nullif(section.value->>'id', ''), 'section-' || section.ordinality::text) as section_id,
      coalesce(nullif(section.value->>'type', ''), 'notes') as type,
  ...
  where s.user_id = v_user_id
    ...
    and (
      s.type is distinct from i.type
      or s.visibility is distinct from i.visibility
      or s.position is distinct from i.position
    );

  -- 4) Remove sections no longer present. FK cascades remove their embeddings.
  ...
  -- Relationships are tiny compared with sections/embeddings; replace them
  -- transactionally to preserve simple conflict semantics.
  delete from public.arc_relationships
  where user_id = v_user_id and from_arc_id = v_arc_id;
